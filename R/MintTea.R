...
#'
#' @param param_sgcca_design A prior on expected relations between different
...
#'   More than one value can be provided if sensitivity analysis is desired.
#'
#' @param param_sgcca_ncomp Number of sCCA components to extract each DIABLO
...
MintTea <- function(
  ...
  if (! is.numeric(param_sgcca_design)) stop('Invalid argument to *param_sgcca_design*')
  if (max(param_sgcca_design) > 1) stop('Invalid argument to *param_sgcca_design* (values should be between 0 to 1)')
  if (min(param_sgcca_design) < 0) stop('Invalid argument to *param_sgcca_design* (values should be between 0 to 1)')
  ...
  if (! is.numeric(param_sgcca_ncomp)) stop('Invalid argument to *param_sgcca_ncomp*')
  if (min(param_sgcca_ncomp) < 0) stop('Invalid argument to *param_sgcca_ncomp* (values should be between 1 to ~10)')
  if (max(param_sgcca_ncomp) > 10) stop('Invalid argument to *param_sgcca_ncomp* (values should be between 1 to ~10)')
  ...
  for (diablo_keepX in param_diablo_keepX) {
    for (diablo_design in param_sgcca_design) {
      for (n_repeats in param_n_repeats) {
        for (n_folds in param_n_folds) {
          for (diablo_ncomp in param_sgcca_ncomp) {
            log_debug(paste(
              diablo_keepX,
              diablo_design,
              n_repeats,
            ...
            feat_pairs <- bind_rows(
              ...
              tmp$feat_pairs %>%
                ...
                mutate(param_sgcca_design = diablo_design) %>%
